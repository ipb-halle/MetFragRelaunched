...
public class FragmentToNonHydrogenFormulaPeakAssigner extends AbstractAssigner {

	private FragmentList fragmentList;
	protected SiriusNodePeakList peakList;
	
	public FragmentToNonHydrogenFormulaPeakAssigner(Settings settings) {
		super(settings);
	...
	public MatchList assign() {
		...
		for(int i = 0; i < this.peakList.getNumberElements(); i++) {
			...
			for(int j = 0; j < fragmentList.getNumberElements(); j++) {
				 ...
				 */
				if(currentFormulaPeak.getMolecularFormula().compareToWithoutHydrogen(fragmentListFormula)) {
					/*

...
public class HDFragmentPairScore2 extends AbstractScore {
	...
	public HDFragmentPairScore2(Settings settings) {
		super(settings);
	...
	public void calculate() {
		...
		else this.value = (double)countedPairs;
	}
				...
				for(int i = 0; i < node.getMatch().getMatchedFragmentsSize(); i++) {
					...
					for(int j = 0; j < numberMatchedFragmentsHD; j++) 
					{
						...
						if(fragment.equals(fragmentHD)) {
							if(numberDeuteriumsOfMatchedFragment == deuteriumShift) return true;
							else if((numberDeuteriumsOfMatchedFragment - deuteriumFromCharge ) == deuteriumShift) return true;
						}
					}
				}

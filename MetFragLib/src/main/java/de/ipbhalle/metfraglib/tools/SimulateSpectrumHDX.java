...
/**
 * generate a simulated spectrum given a candidate and its native spectrum
 * 
 * input: 
 * 
 * parameter file: MetFrag parameter file 
 * output file: where to write output
 * method: 
 * 	1 - [M+D]+
 * 	2 - [M]+
 * 	3 - [M]+ & [M+D]+
 * 	4 - predict masses based on in silico added deuteriums
 * 	5 - predict masses based adduct type [F+H]+ -> [F+D]+
 * 
 * charge: pos or neg
 * 
 * @author chrisr
...
public class SimulateSpectrumHDX {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			if (type.equals("pos"))
				spectrum = generateDeuteratedSpectrumPositive(scoredCandidateList.getElement(0), method);
			else if (type.equals("neg"))
				spectrum = generateDeuteratedSpectrumNegative(scoredCandidateList.getElement(0), method);
	 ...
	 * @throws Exception
	 */
	public static double[][] generateDeuteratedSpectrumPositive(ICandidate candidate, int method)
			throws AtomTypeNotKnownFromInputListException, Exception {
		int numberPositions = searchForDeuteriumExchangeablePositions(
				(TopDownBitArrayPrecursor) candidate.getPrecursorMolecule()).length;
		HDTopDownBitArrayPrecursor preHDX = new HDTopDownBitArrayPrecursor(candidate.getImplicitHydrogenAtomContainer(),
				(byte) numberPositions);
		...
		for (int i = 0; i < matchList.getNumberElements(); i++) {
			...
			// [M]+ [M+D]+
				...
				spectrumHDX[i * 2][0] = peak.getMass() - (numberDeuteriums) * Constants.HYDROGEN_MASS
						+ (numberDeuteriums) * Constants.getMonoisotopicMassOfAtom("D");
				spectrumHDX[(i * 2) + 1][0] = peak.getMass() - (numberDeuteriums) * Constants.HYDROGEN_MASS
						+ (numberDeuteriums) * Constants.getMonoisotopicMassOfAtom("D");
				spectrumHDX[(i * 2) + 1][0] += Constants.getMonoisotopicMassOfAtom("D") - Constants.HYDROGEN_MASS;
			}
			// predict based on in silico added deuteriums
			else if (method == 4) {
				spectrumHDX[i][0] = peak.getMass() - (numberDeuteriums) * Constants.HYDROGEN_MASS
						+ (numberDeuteriums) * Constants.getMonoisotopicMassOfAtom("D");
			...
			// predict based on adduct
				...
				byte adductTypeIndex = ((DefaultFragmentToPeakMatch) matchList.getElement(i))
						.getBestMatchedFragmentAdductTypeIndex();
				spectrumHDX[i][0] = peak.getMass() - (numberDeuteriums) * Constants.HYDROGEN_MASS
						+ (numberDeuteriums) * Constants.getMonoisotopicMassOfAtom("D");
	 ...
	 * @throws Exception
	 */
	public static double[][] generateDeuteratedSpectrumNegative(ICandidate candidate, int method)
			throws AtomTypeNotKnownFromInputListException, Exception {
		int numberPositions = searchForDeuteriumExchangeablePositions(
				(TopDownBitArrayPrecursor) candidate.getPrecursorMolecule()).length;
		HDTopDownBitArrayPrecursor preHDX = new HDTopDownBitArrayPrecursor(candidate.getImplicitHydrogenAtomContainer(),

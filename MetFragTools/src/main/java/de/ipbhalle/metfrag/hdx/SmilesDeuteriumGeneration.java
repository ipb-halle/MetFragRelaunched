...
public class SmilesDeuteriumGeneration {
		...
		for (int j = 0; j < smiles.size(); j++) {
			...
			if (!combinatorial
				...
				for (int k = 0; k < deuteratedStrutures.size(); k++) {
					...
					System.out.println(identifiers.get(j) + "-" + (k + 1) + "|" + inchi + "|"
							+ sg.create(deuteratedStrutures.get(k)) + "|" + formula.toString() + "|"
							+ formula.getMonoisotopicMass() + "|" + inchiKey.split("-")[0] + "|"
							+ inchiKey.split("-")[1] + "|" + numberDeuteriumsEasilyExchangedVec.get(k) + "|0|"
			...
			HDByteMolecularFormula formula = null;
			try {
				formula = new HDByteMolecularFormula(
						MolecularFormulaManipulator.getString(MolecularFormulaManipulator.getMolecularFormula(its)));
			} catch (Exception e) {
				System.err.println(identifiers.get(j));
				e.printStackTrace();
				System.exit(1);
			...
			if (numberToAddDeuteriums.size() == 0 || numberDeuteriums == numberToAddDeuteriums.get(j)) {
				...
				its.setProperty("M+H",
						MathTools.round(mass + Constants.getMonoisotopicMassOfAtom("H") - Constants.ELECTRON_MASS));
				its.setProperty("M+D",
						MathTools.round(mass + Constants.getMonoisotopicMassOfAtom("D") - Constants.ELECTRON_MASS));
				its.setProperty("M-H",
						MathTools.round(mass - Constants.getMonoisotopicMassOfAtom("H") + Constants.ELECTRON_MASS));
				its.setProperty("M-D",
						MathTools.round(mass - Constants.getMonoisotopicMassOfAtom("D") + Constants.ELECTRON_MASS));
				...
				its.setProperty("DeuteratedSMILES",
						MoleculeFunctions.generateSmiles(con).replaceAll("\\[H\\]", "[2H]"));
	...
	public static void printInfo(IAtomContainer jios) {
		for (int i = 0; i < jios.getAtomCount(); i++) {
			...
			if (numDs > 0) {
				System.out.println(jios.getAtom(i).getSymbol() + " " + i + " H:"
						+ getNumberExplicitRealHydrogens(jios, i) + " D:" + getNumberExplicitDeuteriums(jios, i));
			}
		}
	}
